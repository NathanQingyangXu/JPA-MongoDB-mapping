...
public class MongoConnectionProvider implements ConnectionProvider, Configurable, Stoppable, ServiceRegistryAwareService {
    ...
    public void configure(final Map<String, Object> configurationValues) {
        ...
        mongoClient = buildMongoClient(mongodbConnectionURL);
        mongoDatabase = buildMongoDatabase(mongoClient, mongodbDatabaseName);
    ...
    private MongoClient buildMongoClient(final String mongodbConnectionURL) {
        final var connectionString = new ConnectionString(castNonNull(mongodbConnectionURL));
    ...
    private MongoDatabase buildMongoDatabase(final MongoClient mongoClient, final String mongodbDatabaseName) {
        var database = mongoClient.getDatabase(castNonNull(mongodbDatabaseName));
        if (mongoReadWriteOptionsStrategy != null) {
